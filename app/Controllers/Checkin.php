...
class Checkin extends BF_Controller {
	...
	public function login()
	{
		...
		$par_login_email = textinput('par_login_email', '',
			['placeholder'=>'E-Mail',  'inputmode'=>'email', 'style'=>'width: '.($par_left_col_size - 40).'px;',
				'onkeypress'=>'login_press(event);',
				'onkeyup'=>'login_keyup(event);']);
		$par_login_email->persistent('parent');
		...
		$par_code = textinput('par_code', '',
			['placeholder'=>'Kids-ID', 'style'=>'width: '.($par_right_col_size - 40).'px;',
				'onkeypress'=>'login_press(event);',
				'onkeyup'=>'kidsID($(this)); login_keyup(event);']);
		...
		$par_fullname = textinput('par_fullname', '',
			['placeholder'=>'Vorname und Nachname', 'style'=>'width: '.($par_left_col_size - 40).'px;',
				'onkeypress'=>'registration_press(event);',
				'onkeyup'=>'capitalizeName($(this), event); registration_keyup(event);']);
		...
		$par_cellphone = textinput('par_cellphone', '',
			['placeholder'=>'Handy-Nr', 'inputmode'=>'tel', 'style'=>'width: '.($par_right_col_size - 40).'px;',
				'onkeypress'=>'registration_press(event);',
				'onkeyup'=>'registration_keyup(event);']);
		...
		if ($this->have_result()) {
			...
			$this->print_result([ 'style'=>'font-size: 20px;' ]);
			_td();
			_tr();
		}
		tr();
	...
	public function registration()
	{
		...
		script();
		out('
			function parent_enter(ev) {
				var par_fullname = $("#par_fullname").val();
				var par_cellphone = $("#par_cellphone").val();

				if (ev.key == "Enter") {
					ev.preventDefault();
					if (par_fullname == "" || par_cellphone == "") {
						if (par_fullname == "")
							$("#par_fullname").focus();
						else
							$("#par_cellphone").focus();
			...
			function toggle_submit_parent(kid_i, period_count) {
				...
				var par_fullname = $("#par_fullname").val();
				var par_cellphone = $("#par_cellphone").val();
				var par_fullname_before = $("#par_fullname").attr("before");
				var par_cellphone_before = $("#par_cellphone").attr("before");

				if (par_fullname != "" && par_fullname != par_fullname_before)
					changed = true;
				if (par_cellphone != "" && par_cellphone != par_cellphone_before)
					changed = true;

				$("#update_parent").prop("disabled", !changed);
			}
			function kid_enter(kid_i, ev) {
				var kid_fullname = $("#kid_fullname_"+kid_i).val();
				var kid_birthday = $("#kid_birthday_"+kid_i).val();

				if (ev.key == "Enter") {
					ev.preventDefault();
					if (kid_fullname == "" || kid_birthday == "") {
						if (kid_fullname == "")
							$("#kid_fullname_"+kid_i).focus();
						else
							$("#kid_birthday_"+kid_i).focus();
					}
					else
						$("#kid_checkin_form_"+kid_i+" #submit_kid").click();
				}
			}
			function toggle_submit_kid(kid_i, period_count) {
				...
				var kid_fullname = $("#kid_fullname_"+kid_i).val();
				var kid_birthday = $("#kid_birthday_"+kid_i).val();
				var kid_notes = $("#kid_notes_"+kid_i).val();
				var kid_fullname_before = $("#kid_fullname_"+kid_i).attr("before");
				var kid_birthday_before = $("#kid_birthday_"+kid_i).attr("before");
				var kid_notes_before = $("#kid_notes_"+kid_i).attr("before");

				if (kid_fullname != "" && kid_fullname != kid_fullname_before)
					changed = true;
				if (kid_birthday != "" && kid_birthday != kid_birthday_before)
					changed = true;
		...
		$kid_left_col_size, $kid_mid_col_size, $kid_right_col_size)
	{
		...
		$kid_fullname = textinput('kid_fullname_'.$kid_i, $row['kid_fullname'],
			['placeholder'=>'Vorname und Nachname', 'style'=>'width: '.($kid_left_col_size - 40).'px;', 'before'=>$row['kid_fullname'],
				'onkeypress'=>'kid_enter('.$kid_i.', event);',
